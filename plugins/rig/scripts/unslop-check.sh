...
if [ "$#" -gt 0 ]; then
  ...
  # CHANGELOG.md is generated by release-please from commit subjects, so its tells are
  # historical git and unfixable without rewriting history. Naming a path still scans it.
  mapfile -t files < <(git -C "$root" ls-files -co --exclude-standard '*.md' 'docs/*.html' \
    | grep -v '^CHANGELOG\.md$' | sed "s#^#$root/#")
fi
