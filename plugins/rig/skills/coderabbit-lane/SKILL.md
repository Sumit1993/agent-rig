...
- Fixed threads resolve through one verified re-review, never a blanket command. After every fix for the round is committed, pushed and replied to in-thread, stop: the queue re-reviews a PR whose CodeRabbit threads all carry a reply. Summon it yourself only in the §2 cases. CodeRabbit resolves the threads it considers addressed; anything left open is reviewed and resolved individually with rationale in-thread.
