...
Two agents, merged into ONE ranked queue (they overlap — both will flag the CLI reference):
...
1. **`.coderabbit.yaml` path instructions** — the strongest net: the PR diff is where a docs gap is objectively visible. Schema constraint: `reviews.path_instructions[].path` takes a SINGLE glob string — one entry per glob from the registry's `docs.code_globs`, duplicated instruction text. Wording constraint: phrase as "must update the RELEVANT surface, and name the specific stale surface" — "must touch docs/" is satisfiable by any unrelated docs edit. While in there, distill the repo's key invariants into path instructions — this is the only channel by which external-hub design decisions reach CodeRabbit's reviews.
...
Process-side layer: every implementation spec handed to a coding agent gets a "Docs surfaces" deliverable section naming specific files, or an explicit "none affected because…" — replicate in the repo's AGENTS.md.
