...
Features ship and PR descriptions describe them, but the docs silently drift: CLI references, guides, READMEs, `--help` strings, task-runner `desc:` lines. Operators rely on docs, not PR archaeology.

Per-repo parameters come from the kit registry: `kit-meta.sh get <owner/repo> docs` (docs-site dir, surface list, code globs for nets). Verify the surface list against the working tree before using it, and generate audit/fix prompts fresh each run. Never reuse a stored prompt file, because stored prompts rot.

## Illustration standard

**Complex concepts must be shown, not only described.** A passage must carry at least one concrete artifact alongside the prose when it makes the reader hold **three or more moving parts** in their head at once: a resolution order, a mode/topology, a state machine, a pipeline, a precedence rule, or a side-by-side comparison. Prose-only in that situation is a documentation gap, reviewable as one. Same "3+ moving parts" test already used elsewhere in this toolchain. One threshold, not a second competing definition.
...
- Format: `- [ ] Docs: <specific surfaces, comma-separated> updated (or explicitly noted why none apply)`. **Name real files/sections, never a generic "update docs"**, because generic lines get checkbox-ticked without thought.
...
1. **`.coderabbit.yaml` path instructions.** The strongest net, because the PR diff is where a docs gap is objectively visible. Schema constraint: `reviews.path_instructions[].path` takes a SINGLE glob string, so use one entry per glob from the registry's `docs.code_globs`, with duplicated instruction text. Wording constraint: phrase as "must update the RELEVANT surface, and name the specific stale surface". "must touch docs/" is satisfiable by any unrelated docs edit. While in there, distill the repo's key invariants into path instructions. That is the only channel by which external-hub design decisions reach CodeRabbit's reviews. Append the [illustration standard](#illustration-standard) too, verbatim, as its own paragraph in every path entry (same one-glob-per-entry duplication as above):
