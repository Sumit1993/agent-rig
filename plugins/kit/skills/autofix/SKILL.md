...
metadata:
  version: "0.1.0"
  upstream: "coderabbitai/skills autofix"
  upstream_version: "0.1.0"
  upstream_latest_seen: "1.1.1"
  patched: true
  patch_note: "Per-issue replies go IN-THREAD via /replies. Upstream posts a summary comment only, which leaves threads unresolved and blocks merge under required_review_thread_resolution. Story: prismalens PR #160."
  triggers:
    - coderabbit.?autofix
    - coderabbit.?auto.?fix
    - autofix.?coderabbit
...
### Step 10: Reply In-Thread, Then Post Summary

**For EVERY issue fixed, reply INSIDE CodeRabbit's review thread — never only as a new top-level PR comment.** A top-level comment does not resolve the thread; repos using the `required_review_thread_resolution` ruleset stay unmergeable until each thread is resolved, and CodeRabbit resolves its own threads when the fix is confirmed to it in-thread (prismalens PR #160).

For each fixed issue, reply to the thread's root comment:
...
**If at least one fix was applied:** Additionally post one success summary comment on the PR:
