...
# Cells must stay one line, and a leading =, +, -, or @ must not reach a
# spreadsheet as a formula: evidence can carry attacker-controlled text
# (PR titles, filenames, generated output). See skills/show-me-your-work.
clean() {
	local v
	v=$(printf '%s' "$1" | tr '\t\n\r' '   ')
	case "$v" in
