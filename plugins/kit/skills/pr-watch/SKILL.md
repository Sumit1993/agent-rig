...
| Tier | When | What |
|---|---|---|
| Claude review (`claude[bot]`) | Every same-repo PR, automatic, **but not every round, and not every author** | The default reviewer. Posts findings as **inline comments**. Advisory, so it blocks nothing directly, but every thread it opens does. Every run leaves/updates a **liveness comment** (`github-actions[bot]`, marker prefix `<!-- claude-review-liveness`, which carries fields that change over time, `<!-- claude-review-liveness rounds=N sha=<head> -->`; match the prefix only, never the whole string): "posted N" means reviewed; "posted **nothing**" means this head has NO machine review on record. Silence is never approval, and the first-line re-request for it is a summon (`@claude full review`), not a push. Four ways it stays quiet: **(1) skipped author**, a PR whose author is in `skip_authors` (default `dependabot[bot]`) gets no automatic round at all; **(2) auto-paused**, after `auto_pause_rounds` automatic rounds (default 5) the lane pauses itself and the liveness comment reads *auto-paused after N automatic rounds … re-request with `@claude review`*; **a paused PR is NOT reviewed on push, so summon it** (the counter is monotonic in v1, so a summon resumes for exactly that one run); **(3) fork PR**, never machine-reviewed by this lane, it gets a fork-notice comment instead (see below); **(4) self-skip**, a PR editing `claude-code-review.yml` is never reviewed by this lane (a security control, and the one case a summon cannot fix), so label it `coderabbit_review` instead |
| Claude fixer (`@claude fix`) | On demand, org members only, prismalens only (so far) | Applies fixes for **all** unresolved threads on the PR branch and replies in each with the SHA. **One summon comment per round (the @claude fix trigger, not a reply protocol; fix replies always go in-thread).** Each mention is a full agent run. It cannot review, resolve, or merge (tool-denied) |
| Claude review summons (`@claude review`, `@claude full review`) | On demand, org members only (`OWNER`/`MEMBER`/`COLLABORATOR`), **bare PR comment** | The re-review path when no push is coming, and **the only resume for an auto-paused PR**. `@claude review` is *incremental*: it verifies unresolved `claude[bot]` threads and reviews the delta, falling back to a normal review when there are none. `@claude full review` re-reviews **from scratch with dedup disabled**, the fix for a round that finished green having published nothing. Both bypass the draft skip and `skip_authors`; **neither runs on a fork PR** (v1). Same economy as the fixer: **one summon per round**, each a full agent run. **Escalating one run:** `@claude review --model opus` runs that summon on `claude-opus-5`, and `--model sonnet` picks Sonnet back. The lane's `default_model` is `claude-sonnet-5` on purpose, since review is the highest-volume Claude spend across the repos. The override is an allowlist of two exact phrases, so `@claude full review --model opus` does **not** switch models and falls through to the default. Which IDs resolve at all is set by the repo's `CLAUDE_CODE_OAUTH_TOKEN` tier |
| Claude review, fork PRs | Never | A fork head is never machine-reviewed by this lane (secrets are withheld from fork code and the lane deliberately avoids `pull_request_target`). Instead a `fork-notice` job upserts a comment marked `<!-- claude-review-fork-notice -->` saying so and pointing at the `coderabbit_review` label, so the PR is *not* silently quiet. A summon does not override this in v1 |
...
- **`claude[bot]` threads** have no self-resolve mechanism yet: verify the fix, then resolve via GraphQL (`resolveReviewThread`) or the UI, stating so in the reply. The fixer lane is tool-denied from doing this. That denial is load-bearing, don't route around it. After a verification round, run `${CLAUDE_PLUGIN_ROOT}/scripts/resolve-verified.sh <pr>` to batch-resolve reviewer-verified threads.
...
- **`CLAUDE LIVENESS — … auto-paused after N automatic rounds …`: stop waiting, summon.** The lane has hit `auto_pause_rounds` and will not review this PR automatically again. **A watcher waiting for a `claude[bot]` review after a push on a paused PR waits forever.** The push produces no review at all, and the wait has no natural end. The instant that text appears, post a bare `@claude review` (or, if summoning is not yours to do, report the pause and hand it back), never keep waiting on the next push. The same applies to the fork-notice comment (`<!-- claude-review-fork-notice -->`): no Claude review is ever coming on that PR, so route it to `coderabbit_review` or a model pass instead of waiting.
