...
The load-bearing consequence: **on a young OSS repo the CLI counter (3/hr) is larger than the PR counter (~1–2/hr).** So the CLI preview pre-push is not just "nice to catch things early" — it spends the resource you have more of, and protects the one you have least of. The kit plugin's pre-push hook enforces this on registry-enabled repos: `git push` on a non-default branch is blocked until `cr-preview.sh` has run for that branch within 30 minutes (`CR_GATE=skip` in the command overrides, user-approved only).
