...
- **The star count does not tell you whether `auto_review` fires.** Fewer than 10 stars describes the OSS tier's default, and a repo carrying `auto_review: true` gets automatic review anyway. `prismalens/gh-workflows` is public with 0 stars, and `coderabbitai[bot]` posted 8 to 93 seconds after `gh pr create` on `#158`, `#155` and `#142`, with no human summon first. Settle it per repo by comparing the PR's `createdAt` against the first `coderabbitai[bot]` comment in `repos/<owner>/<repo>/issues/<n>/comments`, never by `stargazerCount`. Where it fires on open, the slot is spent at `gh pr create` and again at each push to an unpaused PR, so batching means batching before the push, not before a summon. By the time you would summon, the slot is gone.
