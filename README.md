...
| `skills/coderabbit-lane` | How the independent CodeRabbit lane behaves: managing the scarce shared org-wide counter (~1 review per 40 min), manual admission via `coderabbit_review` label on consumer repos, automatic review on `gh-workflows`, when spending a slot is warranted (path invariants, model cross-check), bare `@coderabbitai review` trigger, in-thread reply protocol with `cr-reply.sh`, rate limits, and thread resolution rules |
