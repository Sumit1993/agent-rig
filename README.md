...
| `skills/unattended-run` | Holding a long unattended run: arm the `CronCreate` wake-up before anything else (it is a deferred tool, it dies with the session, and it only fires while the REPL is idle), then write the plan file; organizer never types, the stall rule ("standing by" = stalled), tear down a watch with its task, green-is-not-evidence, how to change enforcement machinery without taking the repo down, park-don't-decide, when to hand a decision to `fable-planner`, per-tick wake-up checklist |
| `skills/pr-watch` | The session-scoped round on a PR *this session* raised: the merge contract (two required checks + unresolved-thread resolution; reviewers advisory), review tiers (`claude[bot]` → CodeRabbit → Opus → extreme), seed and arm a deterministic reviewer/CI Monitor (zero tokens while quiet), events routed as pointers not payloads, watcher lifecycle. Merge-queue repos enqueue; the old `merge-cascade.sh` BEHIND doctrine is retired. Lane behavior lives in `claude-review-lane` and `coderabbit-lane` |
| `skills/claude-review-lane` | How our own `claude[bot]` lane behaves, on a PR of any age: the liveness comment's marker prefix and four verdicts (only one counts as review evidence), the five ways the lane stays quiet (skipped author, draft, fork head, self-skip on its own workflow, auto-pause) and which of them leave no comment at all, the summon grammar including the per-run `--model` override and the phrase gap that makes `@claude full review --model opus` silently keep the default, verification rounds, and how the reviewer resolves its own verified threads |
| `skills/coderabbit-lane` | How the independent CodeRabbit lane behaves: managing the scarce per-developer counter shared across every repo you touch, manual admission via `coderabbit_review` label on consumer repos, automatic review on `gh-workflows`, when spending a slot is warranted (a sensitive surface, model cross-check), bare `@coderabbitai review` trigger, in-thread reply protocol with `cr-reply.sh`, rate limits, and thread resolution rules |
...
| `skills/tweet` | Drafts 2-3 tweet options for @Desolatte (≤280 chars) from the current session, git log, or mage learnings, applying live voice and dedup context from n8n; hands off via pre-filled `explorer.exe` intent URL. Trigger on /tweet, "tweet this", or requests to share on X |
...
| `hooks/pr-created.sh` | PostToolUse(Bash\|Agent): a successful `gh pr create` injects "arm pr-watch now". Confirms the PR exists over the API first, so a fixture URL in test output does not nudge; an unverifiable check still nudges, marked `UNVERIFIED` |
| `hooks/delegate-check.sh` | PreToolUse(Agent): blocks a generic Claude subagent on work that reads as delegable, since agy draws a separate quota (`AGENTS.md` Delegation). Escape: name agy in the prompt or description |
| `hooks/no-haiku.sh` | PreToolUse(Agent): blocks `model=haiku`. Routing doctrine, never Haiku for real work |
...
| `hooks/organizer-seat.sh` | PreToolUse(Edit\|Write\|NotebookEdit): nudges once per session when the organizer edits files while an agy run is live. Fires only while a delegate is working, since editing with nothing delegated is just doing the work |
| `hooks/release-docs-gate.sh` | PreToolUse(Bash): blocks merging a release PR on a repo whose registry entry has a `docs` block without a docs audit in the last 14 days. Fail-open everywhere. Escape: `DOCS_GATE=skip` in the merge command |
| `hooks/reap-watchers.sh` | SessionEnd and SessionStart: kills watcher processes at session end, and reaps orphans (PPID 1) at start. A monitor outliving its session replays buffered events on resume and that turn pays for the whole context window; seen-state is durable, so killing is free |
