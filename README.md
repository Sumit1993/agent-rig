...
| `skills/unattended-run` | Holding a long unattended run: organizer never types, lane count derived from the actually-scarce resource, the stall rule ("standing by" = stalled), evidence-not-green, rebase-before-push, park-don't-decide, per-tick wake-up checklist |
| `skills/pr-watch` | How the `review-evidence` merge gate is earned (CI-posted, no local pre-push step) + review tiers (CI `claude[bot]` → `coderabbitai[bot]` on high-risk paths → Opus → extreme) + post-PR lifecycle: seed and arm a deterministic CodeRabbit/CI Monitor (zero tokens while quiet), in-thread reply protocol, `merge-cascade.sh` for GitHub auto-merge's BEHIND stranding |
