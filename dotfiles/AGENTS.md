...
| Opus 5.5 (1M) | 4 | 9 | 8 | Session model: the coding, default review |
| Gemini 3.8 Flash | 6 | 8 | 5 | Side work via agy: tests, checks, triage, evidence |
...
# Reviewers
Reviews run async. The hourly CodeRabbit routine (`scripts/coderabbit-routine/` in Sumit1993/rig) summons CodeRabbit on one ready PR per hour across every repo, re-reviews first, and merges a PR whose review came back clean. A draft is never summoned: open drafts, push freely, mark ready once. The Claude lane runs on mage-memory, and on prismalens PRs labelled `claude_review`. Findings wait as review debt for the next session in that repo, which clears them before any new pick. Procedure: `compass`, `coderabbit-lane`, `claude-review-lane`, `pr-babysit`.
...
- Push, open pull requests, create todos, run workflows and spawn subagents without asking. Merge is an explicit per-merge permission, never carried forward, recorded as `MERGE_OK=<pr> gh pr merge <pr>`; a hook refuses the rest. The one standing exception is the CodeRabbit routine merging a PR whose review came back clean.
