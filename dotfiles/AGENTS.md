...
# Reviewers
Reviews run async. The hourly review queue (`review-queue.yml` in prismalens/gh-workflows) summons CodeRabbit on one ready PR per hour across every repo, re-reviews first, and merges a PR whose review came back clean. A draft is never summoned: open drafts, push freely, mark ready once. The Claude lane runs on mage-memory, and on prismalens PRs labelled `claude_review`. Findings wait as review debt for the next session in that repo, which clears them before any new pick. Procedure: `compass`, `coderabbit-lane`, `claude-review-lane`, `pr-babysit`.
