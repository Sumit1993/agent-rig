...
# frontmatter differ from Claude Code's. Untagged means Claude-only. Story: claude-kit#134.
set -euo pipefail
SRC="$(cd "$(dirname "$0")/../plugins/kit" && pwd)"
...
jq -n --arg d "$(jq -r .description "$SRC/.claude-plugin/plugin.json")" \
  '{"$schema": "https://antigravity.google/schemas/v1/plugin.json", name: "kit", description: $d}' > "$OUT/plugin.json"
